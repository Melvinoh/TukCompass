...
export const userRegistration = async (req, res) =>{
    ...
    try {
        const{ userID, fname, sname, email, password, role, mobile, courseID, classID, departmentID}= req.body
        ...
        if (role === 'student') {
           ...
           await Student.create({studentID : userID,},{transaction : t})
            
        }else if (role === 'lecturer') {
            await Lecturer.create({lecturerID: userID,})
        }
        await t.commit()
